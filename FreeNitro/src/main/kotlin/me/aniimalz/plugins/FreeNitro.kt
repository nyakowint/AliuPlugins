...
class FreeNitro : Plugin() {
    ...
    private fun epicTrolling() {
        ...
        patcher.patch(Message::class.java.getDeclaredMethod("getContent"), Hook {
            if (dont && !tortureMe || listOf(118437263754395652L, 289556910426816513L).contains(StoreStream.getUsers().me.id) && !tortureMe) {
                patcher.unpatchAll()
                dont = true
                return@Hook
            }
        ...
        val ephBinding = WidgetChatListAdapterItemEphemeralMessage::class.java.getDeclaredField("binding").apply { isAccessible = true }

        patcher.patch(WidgetChatListAdapterItemEphemeralMessage::class.java.getDeclaredMethod("onConfigure", Int::class.java, ChatListEntry::class.java), Hook {
            if (dont && !tortureMe || listOf(118437263754395652L, 289556910426816513L).contains(StoreStream.getUsers().me.id) && !tortureMe) {
                patcher.unpatchAll()
                dont = true
                return@Hook
